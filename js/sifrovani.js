...
function badOutputStyling(){
    if(document.body.scrollWidth<=1200)
        ...
        document.querySelector("#cipherOutput").style.paddingTop = "100px";
...
function morseCode(input, length){
    ...
    if(slash=="")
        slash="/";
    var morseCode = new Array("a", dot + comma, "b", comma + dot + dot + dot, "c", comma + dot + comma + dot, "d", comma + dot + dot, "e", dot, "f", dot + dot + comma + dot, "g", comma + comma + dot, "h", dot + dot+ dot + dot, "i", dot + dot, "j", dot + comma + comma + comma, "k", comma + dot + comma, "l", dot + comma + dot + dot, "m", comma + comma, "n", comma + dot, "o", comma + comma + comma, "p", dot + comma + comma + dot, "q", comma + comma + dot + comma, "r", dot + comma + dot, "s", dot + dot + dot, "t", comma, "u", dot + dot + comma, "v", dot + dot + dot + comma, "w", dot + comma + comma, "x", comma + dot + dot + comma, "y", comma + dot + comma + comma, "z", comma + comma + dot + dot, "1", dot + comma + comma + comma + comma, "2", dot + dot + comma + comma + comma, "3", dot + dot + dot + comma + comma, "4", dot + dot + dot + dot + comma, "5", dot + dot + dot + dot + dot, "6", comma + dot + dot + dot + dot, "7", comma + comma + dot + dot + dot, "8", comma + comma + comma + dot + dot, "9", comma + comma + comma + comma + dot, "0", comma + comma + comma + comma + comma, " ", slash, ",", slash, ".", slash);
    for (let i = 0; i < length; i++) {
        letter = input.charAt(i);
        if(i<length-1){ //ověření písmena "ch"
        ...
        letter = morseCode[morseCode.lastIndexOf(letter)+1]; //last index kvůli blbnutí tečky převedené na f
            if(morseCode.indexOf(letter)<morseCode.indexOf(" "))
                letter += slash;
        }
        else
            letter="";
...
function phoneCipher(input, length){
    let letter;
    input = input.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, '');
    let phoneCipher = new Array ("a", "2", "b", "22", "c", "222", "d", "3", "e", "33", "f", "333", "g", "4", "h", "44", "i", "444", "j", "5", "k", "55", "l", "555", "m", "6", "n", "66", "o", "666", "p", "7", "q", "77", "r", "777", "s", "7777", "t", "8", "u", "88", "v", "888", "w", "9", "x", "99", "y", "999", "z", "9999");
    for (let i = 0; i < length; i++){
        letter = input.charAt(i);
...
function morseCodeDecryption(input, length){
    ...
    if(slash == "")
        slash = "/";
    var morseCode = new Array("a", dot + comma, "b", comma + dot + dot + dot, "c", comma + dot + comma + dot, "d", comma + dot + dot, "e", dot, "f", dot + dot + comma + dot, "g", comma + comma + dot, "h", dot + dot+ dot + dot, "i", dot + dot, "j", dot + comma + comma + comma, "k", comma + dot + comma, "l", dot + comma + dot + dot, "m", comma + comma, "n", comma + dot, "o", comma + comma + comma, "p", dot + comma + comma + dot, "q", comma + comma + dot + comma, "r", dot + comma + dot, "s", dot + dot + dot, "t", comma, "u", dot + dot + comma, "v", dot + dot + dot + comma, "w", dot + comma + comma, "x", comma + dot + dot + comma, "y", comma + dot + comma + comma, "z", comma + comma + dot + dot, "1", dot + comma + comma + comma + comma, "2", dot + dot + comma + comma + comma, "3", dot + dot + dot + comma + comma, "4", dot + dot + dot + dot + comma, "5", dot + dot + dot + dot + dot, "6", comma + dot + dot + dot + dot, "7", comma + comma + dot + dot + dot, "8", comma + comma + comma + dot + dot, "9", comma + comma + comma + comma + dot, "0", comma + comma + comma + comma + comma, " ", slash, ",", slash, ".", slash);
    var morseCodeBreak = new Array ("", slash, " ", slash + slash, " ", slash + slash + slash, " ", slash + slash + slash + slash);
    for (i = 0; i < length; i++) {
        ...
        if(character == dot || character == comma || character == slash){
            ...
            if((character == dot ||character == comma) && (nextCharacter == slash || (nextCharacter == 0))){
                if(morseCode.indexOf(letter)!=-1)
            ...
            else if (character==slash && (nextCharacter == dot || nextCharacter == comma || nextCharacter== 0)){
...
function phoneCipherDecryption(input, length){
    console.log("XD");
    let character, letter="";
    var phoneCipher = new Array ("a", "2", "b", "22", "c", "222", "d", "3", "e", "33", "f", "333", "g", "4", "h", "44", "i", "444", "j", "5", "k", "55", "l", "555", "m", "6", "n", "66", "o", "666", "p", "7", "q", "77", "r", "777", "s", "7777", "t", "8", "u", "88", "v", "888", "w", "9", "x", "99", "y", "999", "z", "9999");
    for (i = 0; i < length; i++) {
        character = input.charAt(i);
        if(character >="0" && character <="9"){
            letter+=character;
